...
SEGMENT_PROMPT = """请将以下内容按语义段落划分。你的目标是识别真正的章节/主题边界，而不是按空行或编号机械拆分。

规则：
1. 只在真正的主题转换处插入分隔符 {delimiter}（如章节切换、话题明显转变）
2. 属于同一主题的连续内容（如多个连续编号的习题、一个章节的多个段落）合并为一个段落
3. 分隔符独占一行
4. 不要修改原文的任何文字、标点和格式
5. 不要添加任何说明、标题或解释
6. 如果全文只有一个自然段落，则不需要插入分隔符
...
应该分成 1 段（同主题习题合并），而不是 3 段。

原文：
{text}"""


def llm_segment_text(text: str, llm_call: Callable[[str], str]) -> str:
    """Send text to LLM for semantic segmentation, returns text with delimiters inserted."""
    prompt = SEGMENT_PROMPT.format(delimiter=SEGMENT_DELIMITER, text=text)
    return llm_call(prompt)


def parse_llm_segments(segmented_text: str) -> list[str]:
    """Parse LLM response into paragraph segments."""
    # Remove the original text if LLM echoes it, keep only the segmented version
    paragraphs = re.split(rf"\s*{re.escape(SEGMENT_DELIMITER)}\s*", segmented_text.strip())
    return [p.strip() for p in paragraphs if p.strip()]


def build_llm_manifest(text: str, llm_call: Callable[[str], str], max_chunk_size: int = 2000) -> ChunkManifest:
    """Build manifest using LLM-based semantic segmentation instead of hard splitting.

    GLM identifies semantic boundaries first. If a segment is too long (> max_chunk_size),
    it's further split by sentence boundaries so each chunk stays rewrite-friendly.
    """
    segmented = llm_segment_text(text, llm_call)
    paragraphs = parse_llm_segments(segmented)

    # Fallback: if LLM didn't split anything, use the full text as one paragraph
    if len(paragraphs) <= 1:
        paragraphs = [text]
    ...
    for paragraph_index, paragraph in enumerate(paragraphs):
        if len(paragraph) <= max_chunk_size:
            ...
            chunk_texts = split_paragraph_to_chunks(paragraph, chunk_limit=max_chunk_size, chunk_metric="char")
        chunk_ids: list[str] = []
        for chunk_index, chunk_text in enumerate(chunk_texts):
            chunk_id = f"p{paragraph_index}_c{chunk_index}"
            chunk_ids.append(chunk_id)
