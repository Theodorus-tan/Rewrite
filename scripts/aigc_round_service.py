...
def run_round(
    ...
    based_on_output_path: str | None = None,
    based_on_manifest_path: str | None = None,
    segment_fn: Callable[[str], ChunkManifest] | None = None,
) -> dict:
    ...
    if segment_fn is not None:
        if progress_callback is not None:
            progress_callback({
                ...
                "totalChunks": 0,
                "completedChunks": 0,
                "message": f"全文 {len(text)} 字，GLM 语义分段中…",
            })
        manifest = segment_fn(text)
    else:
    ...
    for index, chunk in enumerate(manifest.chunks, start=1):
        ...
        if progress_callback is not None:
            # Show first 80 chars of the chunk being processed
            input_preview = chunk.text[:80].strip()
            progress_callback(
                {
                    ...
                    "inputPreview": input_preview + ("…" if len(chunk.text) > 80 else ""),
                }
            )
        try:
        ...
        if progress_callback is not None:
            c_out = chunk_outputs.get(chunk.chunk_id, "") or ""
            output_preview = c_out[:120].strip()
            progress_callback(
                {
                    ...
                    "chunkId": chunk.chunk_id,
                    "progressPath": str(normalized_progress_path),
                    "outputPreview": output_preview + ("…" if len(c_out) > 120 else ""),
                }
            )
